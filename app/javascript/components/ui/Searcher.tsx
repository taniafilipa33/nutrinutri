...
export default function Searcher({ onSearch }: SearcherProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col justify-center md:flex-row gap-2 md:gap-4 bg-gradient-to-r from-teal-400 to-green-400 p-4 rounded-lg items-center">
            <input type="text" onChange={(e) => setQuery(e.target.value)} placeholder="Search..." className="w-full md:w-72 px-4 py-2 rounded-md bg-white text-gray-700 placeholder-gray-400 shadow-sm outline-none" />
            <div className="relative w-full md:w-72">
                <input type="text" onChange={(e) => setLocation(e.target.value)} placeholder="Location..." className="w-full px-4 py-2 rounded-md bg-white text-gray-700 placeholder-gray-400 shadow-sm outline-none" />
            </div>
            <button
                type="submit"
